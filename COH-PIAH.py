...
import re #Regular Expressinons, biblioteca disponivel para fazer o exercício

def ler_assinatura():
  
#A função serve para dar entrada nos valores de traço linguistico e devolver uma assinatura comparada com os textos colocado em análise

  
  print("Bem-vindo ao detector automático de COH-PIAH.")

  pal = float(input("Digite o tamanho médio de palavra:"))
  tok = float(input("Digite a relação Type-Token:"))
  hap = float(input("Digite a Razão Hapax Legomana:"))
  set = float(input("Digite o tamanho médio de sentença:"))
  com = float(input("Digite a complexidade média da sentença:"))
  fra = float(input("Digite o tamanho médio de frase:"))

  return [pal, tok, hap, set, com, fra] #retorna os parâmetros pré estabelecidos
  
def ler_textos(): #Essa função serve para dar entrada nos textos que serão comparados por meio de uma lista vazia utilizando o .append [texto 1, texto 2]
    i = 1
    textos = []
    texto = input("Digite o texto " + str(i) +" (aperte enter para sair):")
    while texto:                                                                   #enquntao texto não for vazio vai continuar repitindo
        textos.append(texto)
        i += 1
        texto = input("Digite o texto " + str(i) +" (aperte enter para sair):")
        
    return textos #retorna a lista contendo o texto 1 e o texto 2

def separa_sentencas(texto): #vai ser responsável por procurar as sentenças presentes no texto por meio do import realizado
...
def separa_frases(sentenca): #A funcao recebe uma sentenca e devolve uma lista das frases dentro da sentenca
    
    return re.split(r'[,:;]+', sentenca)

def separa_palavras(frase): #A funcao recebe uma frase e devolve uma lista das palavras dentro da frase
    
    return frase.split()

def n_palavras_unicas(lista_palavras): #Essa funcao recebe uma lista de palavras e devolve o numero de palavras que aparecem uma unica vez
...
def n_palavras_diferentes(lista_palavras): #Essa funcao recebe uma lista de palavras e devolve o numero de palavras diferentes utilizadas
...
def compara_assinatura(as_a, as_b):  # IMPLEMENTAR. Essa funcao recebe duas assinaturas de texto e deve devolver o grau de similaridade nas assinaturas.
...
def main():
    assinatura_cp = ler_assinatura() #lê a assinatura do aluno infectado com COH-PIAH e retorna a assinatura, que é uma lista contendo os 6 traços linguísticos
    textos_lidos = ler_textos()  #lê os textos e retorna uma lista de textos que serão comparados com a assinatura do aluno infectado com COH-PIAH
